...
class DataTransformation:
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            train_df = pd.read_csv(r"train_path")
            test_df = pd.read_csv(r"test_path")
            ...
            input_feature = train_df.drop(columns=[target_column_name], axis = 1)
            ...
            input_feature = preprocessing_obj.fit_transform(input_feature)
            input_feature_test = preprocessing_obj.transform(input_feature_test)

            train_arr = np.c_[
                input_feature,np.array(target_feature_train_df)
            ]
            
            test_arr = np.c_[
                input_feature,np.array(target_feature_test_df)
            ]
